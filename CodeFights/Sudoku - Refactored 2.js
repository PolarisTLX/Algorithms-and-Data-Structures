function sudoku2(grid) {
    ...
    for (let i = 0; i < grid.length; i++) {
        ...
        for (let j = 0; j < grid.length; j++) {
           if (grid[i][j] != '.') {
               ...
               if (hashCheck[grid[i][j]]) { return false; }
               hashCheck[grid[i][j]] = 1;
    ...
    for (let k = 0; k < grid.length; k++) {
        ...
        for (let l = 0; l < grid.length; l++) {
          if (grid[l][k] != '.') {
               // if value does not yet exist
               if (hashCheck[grid[l][k]]) { return false; }
               hashCheck[grid[l][k]] = 1;                  } } }
    ...
    for (let x = 0; x < 9; x+=3) {
        for (let y = 0; y < 9; y+=3) {
            ...
            for (let m = x; m < x+3; m++) {
                for (let o = y; o < y+3; o++) {
                    if (grid[m][o] != '.') {
                        if (hashCheck[grid[m][o]]) { return false; }
                        hashCheck[grid[m][o]] = 1;                   } } } } }
